...
public class Track : ScriptableObject
{
	...
	public override string ToString ()
	{
		...
		if (System.String.IsNullOrEmpty(Name)) { NameString = name; }
		return NameString;
	}
    ...
    public void PlayTrack(float FadeDuration = 0f, bool IsOneShot = false)
    {
        ...
        //Starts main Edition
        else { SwitchEdition(0, FadeDuration); }
    }
    ...
    {
        if (EditionIndex >= 0 && EditionIndex < Editions.Length) { SwitchEdition(Editions[EditionIndex], Editions[EditionIndex].FadeLength); }
        else { Debug.LogError("The desired Edition could not be started as the current Track only contains " + Editions.Length.ToString() + " Edition" + (Editions.Length == 1 ? "." : "s.")); }
    }
    /// <summary>Switches the Edition of the track to be played.</summary>
    /// <param name="EditionIndex">The index of the desired Edition in the Track's array of Editions.</param>
    ...
    {
        if (EditionIndex >= 0 && EditionIndex < Editions.Length) { SwitchEdition(Editions[EditionIndex], FadeOverride); }
        else { Debug.LogError("The desired Edition could not be started as the current Track only contains " + Editions.Length.ToString() + " Edition" + (Editions.Length == 1 ? "." : "s.")); }
    }
    /// <summary>Switches the Edition of the track to be played.</summary>
    /// <param name="DesiredEdition">The desired Edition to switch to.</param>
    public void SwitchEdition(Edition DesiredEdition) { SwitchEdition(DesiredEdition, DesiredEdition.FadeLength); }
    /// <summary>Switches the Edition of the track to be played.</summary>
    /// <param name="DesiredEdition">The desired Edition to switch to.</param>
    /// <param name="FadeOverride">The duration of the crossfade effect.</param>
    public void SwitchEdition(Edition DesiredEdition, float FadeOverride)
    {
        if (CurrentEdition != DesiredEdition)
        {
            ...
            if (!CurrentEdition.IsNull)
            {
                ...
                if (DesiredEdition.FadeLength > 0) { MusicManager.Main.FadeAudio(CurrentEdition.Audio, DesiredEdition.FadeLength, 0f); ActiveAudio.Remove(CurrentEdition.Audio); }
                else { Stop(CurrentEdition.Audio); }
            }
    ...
    public void StopTrack() { StopTrack(false, 1f); }
    /// <summary>Stops the track and all the audio associated with it.</summary>
    /// <param name="FadeDuration">The duration of the fade effect.</param>
    public void StopTrack(float FadeDuration) { StopTrack(true, FadeDuration); }
    /// <summary>Stops the track and all the audio associated with it.</summary>
    /// <param name="Fade">Whether to fade it out or not.</param>
    public void StopTrack(bool FadeDuration) { StopTrack(FadeDuration, 1f); }
    /// <summary>Stops the track and all the audio associated with it.</summary>
    /// <param name="Fade">Whether to fade it out or not.</param>
    /// <param name="FadeDuration">The duration of the fade effect.</param>
    public void StopTrack(bool Fade, float FadeDuration)
    {
        ...
        {
            if (Fade) { MusicManager.Main.FadeAudio(ActiveAudio[i], FadeDuration, 0f); }
            else { MusicManager.Main.StopAudio(ActiveAudio[i]); }
        }
        ActiveAudio.Clear();
