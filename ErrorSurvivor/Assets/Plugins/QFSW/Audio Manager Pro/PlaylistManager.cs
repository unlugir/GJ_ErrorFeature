...
using UnityEngine;
using AMPInternal;
using AMPInternal.Coroutines.Music;
...
public class PlaylistManager : MonoBehaviour
{
    ...
    private void ValidateIsPlaying(Track CurrentManagerTrack)
    {
        ...
        if (CurrentManagerTrack != TargetPlaylist.CurrentTrack) { StopAllJobs(); }
    }
    ...
    {
        if (TargetPlaylist == null) { return; }
        if ((CurrentManagerTrack != TargetPlaylist.CurrentTrack && CurrentManagerTrack != TargetPlaylist.LastTrack) || !IsOneShot) { StopAllJobs(); }
    }
    ...
    {
        if (Delay > 0) { DelayJob = StartCoroutine(DelayedPlay(DesiredPlaylist, Delay)); }
        else
        {
            //Resets
            StopAllJobs();
    ...
    private IEnumerator PlayPlaylist()
    {
        ...
        { 
            ...
            if (TargetPlaylist != null && TargetPlaylist.IsPlaying && (MusicManager.Main.IsPlaying(TargetPlaylist.CurrentTrack) || !MusicManager.Main.IsPlaying())) { TargetPlaylist.PlayNext(); }
            else { Stop(); }
        }
    }
    ...
    private Playlist FindPlaylistInLibrary(string PlaylistName)
    {
        ...
        if (DesiredPlaylist == null) { Debug.LogError("No Playlist named '" + PlaylistName + "' was found in the PlaylistLibrary of the PlaylistManager."); }
    ...
    public Playlist PlayFromLibrary(string PlaylistName, float Delay = 0)
    {
        ...
        if (DesiredPlaylist != null) { Play(DesiredPlaylist, Delay); }
        return DesiredPlaylist;
    }
}
