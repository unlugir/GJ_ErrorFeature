...
namespace AMPInternal.Editor.Music
{
    ...
    public class PlaylistInspector : UnityEditor.Editor
    {
        ...
        /// <summary>Various different styles for colouring the background.</summary>
        private GUIStyle[] BackgroundStyles;
        ...
        private void CreateStyles()
        {
            if (!InitialisedStyles)
            {
                ...
                //Creates background styles
                BackgroundStyles = new GUIStyle[2];
                for (int i = 0; i < BackgroundStyles.Length; i++)
                {
                    BackgroundStyles[i] = new GUIStyle();
                    BackgroundStyles[i].normal.background = new Texture2D(1, 1);
                    BackgroundStyles[i].stretchHeight = true;
                    BackgroundStyles[i].stretchWidth = true;
                }
                BackgroundStyles[0].normal.background.SetPixel(0, 0, new Color(0.3f, 0.6f, 1f, 0.35f));
                BackgroundStyles[1].normal.background.SetPixel(0, 0, new Color(0.2f, 0.4f, 0.7f, 0.5f));
                for (int i = 0; i < BackgroundStyles.Length; i++) { BackgroundStyles[i].normal.background.Apply(); }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            //Crossfade settings
            GUILayout.BeginVertical(BackgroundStyles[0], GUILayout.Height((PlaylistInstance.Crossfade == Playlist.CrossfadeMode.ForcedOn ? 2 : 1) * EditorGUIUtility.singleLineHeight));
