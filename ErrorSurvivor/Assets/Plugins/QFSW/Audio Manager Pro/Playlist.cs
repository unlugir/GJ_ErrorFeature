...
public class Playlist : ScriptableObject
{
	...
	public override string ToString ()
	{
		...
		if (System.String.IsNullOrEmpty(PlaylistName)) { NameString = name; }
		return NameString;
	}
    ...
    public void Start()
    {
        ...
        switch (Shuffle)
        {
            case ShuffleMode.Random: TrackID = TracksToPlay.RandomIndex(); break;
            case ShuffleMode.RandomUnique: TracksToPlay.Randomize(); goto case ShuffleMode.None;
            case ShuffleMode.None: TrackID = 0; break;
        }
        NextTrackID = TrackID;
    ...
    public float CalculateWaitTime()
    {
        ...
        switch (Crossfade)
        {
            ...
            case CrossfadeMode.ForcedOn: WaitTime -= CrossfadeDuration; break;
            case CrossfadeMode.TrackDefined: WaitTime -= TracksToPlay[NextTrackID].Editions[0].FadeLength; break;
            case CrossfadeMode.ForcedOff: WaitTime += TrackGapDuration; break;
        }
        return WaitTime;
    }
    ...
    public void PlayNext()
    {
        ...
        switch (Crossfade)
        {
            case CrossfadeMode.ForcedOn: MusicManager.Main.Play(CurrentTrack, 0, CrossfadeDuration, true); break;
            case CrossfadeMode.ForcedOff: MusicManager.Main.Play(CurrentTrack, 0, 0, true); break;
            case CrossfadeMode.TrackDefined: MusicManager.Main.Play(CurrentTrack, 0, true); break;
        }
    ...
    private void SelectNext()
    {
        ...
        if (Tracks.Length == 1) { NextTrackID = 0; }
        else
        {
            if (Shuffle == ShuffleMode.Random) { while (TrackID == NextTrackID) { NextTrackID = TracksToPlay.RandomIndex(); } }
            else
            {
                NextTrackID++;
                if (NextTrackID >= TracksToPlay.Count)
                {
                    ...
                    if (Shuffle == ShuffleMode.RandomUnique) { do { TracksToPlay.Randomize(); } while (TracksToPlay[NextTrackID] == CurrentTrack); }
                }
            }
        }
    }
